...
class Movie():
    ...
    def parse(self, soup, url):
        ...
        try:
            tmp.country = \
            re.findall('<span class="pl">制片国家/地区:</span>(\D*)<br />\n<span class="pl">语言:</span>', str(soup))[0].strip()
            print(tmp.country)
        except:
        ...
        try:
            tmp.duration = str(
                re.findall('(\d*)[\S\s]*', soup.findAll('span', {'property': 'v:runtime'})[0].contents[0])[0])
            print(tmp.duration)
        except:
        ...
        try:
            tmp.abstract = ''.join(
                [str(foo).strip() for foo in soup.findAll('span', {'property': 'v:summary'})[0].contents if
                 str(foo) != '<br />'])
            print(tmp.abstract)
        except:
...
class Book():
    ...
    def parse(self, soup, url):
        ...
        # author
        try:
            author_list_html = re.split('\s+/\s+',
                                        re.findall('<span class="pl"> 作者</span>:([\S\s]*)<span class="pl">出版社:</span>',
                                                   str(soup))[0])
            author_list = [re.findall('<a class="" href="/search/\S*">([\S\s]*)</a>', foo)[0].strip() for foo in
                           author_list_html]
        ...
        try:
            tmp.star = str(
                soup.findAll('strong', {'class': 'll rating_num ', 'property': 'v:average'})[0].contents[0].strip())
        ...
        # abstract
        try:
            tmp.abstract = str(''.join(map(str, soup.findAll('div', {'class': 'intro'})[0].contents))).replace('<p>',
                                                                                                               '').replace(
                '</p>', '').replace('\n', '').replace('\r', '').strip()
            print(tmp.abstract)
        except:
...
class Doulist():
    ...
    def retrive(self):
        ...
        while (1):
            ...
            ItemCrawler = Crawler()
            ItemCrawler.url = 'http://www.douban.com/doulist/%s/?start=%d&sort=seq' % (
            self.doulist_id, self.url_id * self.url_increment)
...
class Taglist():
    ...
    def retrive(self):
        ...
        while (1):
            ...
            ItemCrawler = Crawler()
            if self.item_type == 'movies':
                ItemCrawler.url = 'http://movie.douban.com/tag/%s?start=%d&type=T' % (
                urllib.parse.quote(self.tag), self.url_id * self.url_increment)
                ItemCrawler.pattern = {'name': 'table', 'attrs': {'width': '100%'}}
            else:
                ItemCrawler.url = 'http://book.douban.com/tag/%s?start=%d&type=T' % (
                urllib.parse.quote(self.tag), self.url_id * self.url_increment)
